...
type ReconciliationReviewContext = { accountId: string; dateFrom: string; dateTo: string };
...
function validQueryDate(value: string) {
  return /^\d{4}-\d{2}-\d{2}$/.test(value);
}

export function transactionViewContextFromQuery(query: Query) {
  const accountId = /^\d+$/.test(queryValue(query, 'account_id')) ? queryValue(query, 'account_id') : '';
  const dateTo = validQueryDate(queryValue(query, 'date_to')) ? queryValue(query, 'date_to') : '';
  const candidateDateFrom = validQueryDate(queryValue(query, 'date_from')) ? queryValue(query, 'date_from') : '';
  const dateFrom = candidateDateFrom && dateTo && candidateDateFrom > dateTo ? '' : candidateDateFrom;
  const status: Filters['status'] = queryValue(query, 'status') === 'posted' ? 'posted' : '';
  const review = queryValue(query, 'review') === 'reconciliation' && Boolean(accountId && dateTo)
...
export function TransactionsView({ query = {} }: { query?: Query }) {
  const { selectedSlug } = useTenant();
  const context = transactionViewContextFromQuery(query);
  return <TransactionsTenantView key={selectedSlug ?? 'no-tenant'} tenantSlug={selectedSlug} initialFilters={context.filters} reviewContext={context.review} />;
}

function TransactionsTenantView({ tenantSlug, initialFilters, reviewContext }: { tenantSlug: string | null; initialFilters: Filters; reviewContext: ReconciliationReviewContext | null }) {
  ...
  useEffect(() => {
    let active = true;
    let storedIds = new Set<number>();
    try {
      const stored = JSON.parse(reviewStorageKey ? sessionStorage.getItem(reviewStorageKey) ?? '[]' : '[]');
      storedIds = new Set(Array.isArray(stored) ? stored.filter((id): id is number => Number.isInteger(id)) : []);
    } catch { /* Start with an empty temporary checklist. */ }
    queueMicrotask(() => { if (active) setReviewedIds(storedIds); });
    return () => { active = false; };
  }, [reviewStorageKey]);
  ...
  const markReviewed = (transactionId: number, checked: boolean) => {
    setReviewedIds((current) => {
      ...
      if (reviewStorageKey) {
        try { sessionStorage.setItem(reviewStorageKey, JSON.stringify([...next])); } catch { /* Temporary progress can remain in memory. */ }
      }
      return next;
    });
  };
  const clearReviewed = () => {
    ...
    if (reviewStorageKey) {
      try { sessionStorage.removeItem(reviewStorageKey); } catch { /* Temporary progress can remain cleared in memory. */ }
    }
  };
  const reviewAccount = reviewContext ? accounts.find((account) => String(account.id) === reviewContext.accountId) : null;
  ...
  return <Stack spacing={3}>
    <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'stretch', sm: 'center' }} gap={2}>
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>{!reviewContext && <Button variant="outlined" onClick={reviewUncategorized}>Review uncategorized</Button>}<Button variant="contained" startIcon={<AddRounded />} onClick={openCreate} disabled={!tenantSlug || accountsLoading || Boolean(accountsError) || accounts.length === 0}>Add transaction</Button></Stack>
    </Stack>
    ...
    {reviewContext && <Alert severity="info">
      <Stack spacing={1.25}>
        <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', sm: 'center' }} gap={1}>
          <Box><Typography fontWeight={700}>Statement reconciliation review</Typography><Typography variant="body2">{reviewAccount?.name ?? 'Selected account'} · {reviewContext.dateFrom ? `${formatDateOnly(reviewContext.dateFrom)} – ` : 'Through '}{formatDateOnly(reviewContext.dateTo)}</Typography></Box>
          {meta && <Chip color={reviewedIds.size >= meta.total && meta.total > 0 ? 'success' : 'default'} label={`${Math.min(reviewedIds.size, meta.total)} of ${meta.total} checked`} />}
        </Stack>
        <Typography variant="body2">Check each posted transaction after locating it on the bank statement. This checklist is temporary in this browser tab and does not change MG5 records.</Typography>
        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap><Button size="small" variant="outlined" onClick={clearReviewed} disabled={reviewedIds.size === 0}>Clear checks</Button><Button size="small" component={Link} href="/dashboard/reconciliation">Back to reconciliation</Button><Button size="small" component={Link} href="/dashboard/transactions" color="inherit">Exit statement review</Button></Stack>
      </Stack>
    </Alert>}
